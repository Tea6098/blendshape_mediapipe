...
public class Snapshot : MonoBehaviour
{
   ...
   readonly int _vertTop = 10;
   readonly int _vertBot = 152;

   readonly int _vertLeft=234;
   readonly int _vertRight=454;

   readonly int _vertFront=1;

  [SerializeField] PuppetControl puppet;
  
  [SerializeField] string _loadVertsCSV;
        /* #region Triangles */
  ...
  /* #endregion */
    ...
    //Mediapipe.NormalizedLandmarkList _lndmrks;
  ...
  Vector3 AsVector3(Mediapipe.NormalizedLandmark l){
    return new Vector3(l.X,l.Y,l.Z);

  }


  private bool IsHigher(Vector3 a, Vector3 b) => a.y>b.y ?true:false;
  bool IsToTheLeft(Vector3 a, Vector3 b) =>a.x<b.x? true:false;
   bool IsFrontToBack(Vector3 a, Vector3 b)=>a.z<b.z? true:false;
  private List<Vector3> CenterRotateNormalizeFlip(List<Vector3> verts, Quaternion faceRot)
  {

    for (var vi = 0; vi < verts.Count; vi++)
    {
      verts[vi] = Quaternion.Inverse(faceRot) * verts[vi];
      //  verts[vi] = (faceRot) * verts[vi];
    }
    ...
    var currUp = verts[_vertTop] - verts[_vertBot];
    var rot = Quaternion.FromToRotation(currUp, Vector3.up);
    for (var vi=0; vi<verts.Count;vi++)
    {
      verts[vi] = rot * verts[vi];
    }

    var currLeft = verts[_vertLeft] - verts[_vertRight];
    rot = Quaternion.FromToRotation(currLeft, Vector3.left);
    for (var vi=0; vi<verts.Count;vi++)
    {
      verts[vi] = rot * verts[vi];
    }
    ...
    */
    ...
    //flip Z
    if (IsFrontToBack(verts[_vertFront], verts[_vertLeft]))
    {
      for (var vi=0; vi<verts.Count; vi++)
      {
        verts[vi] = new Vector3(verts[vi].x, verts[vi].y, -verts[vi].z);
...
*/
  ...
  void OnGUI()
  {
    ...
    if (Event.current.isKey && Event.current.type == EventType.KeyDown)
    {
      ...
      {
        _verts.Clear();
        var lndmrks = puppet.GetFaceVertsLocal();

        for (int i = 0; i < lndmrks.Count; i++)
        {
          lndmrks[i] = Vector3.Scale(lndmrks[i], new Vector3(0.01f, 0.01f, 0.01f));
        }

        var mesh = new Mesh();
        mesh.name = "face";

        lndmrks = CenterRotateNormalizeFlip(lndmrks, transform.rotation);

        mesh.SetVertices(lndmrks);
        ...
        g.GetComponent<Renderer>().material  = new Material(Shader.Find("Diffuse"));
        MeshFilter mf = g.GetComponent<MeshFilter>();
        mf.sharedMesh = mesh;


        MeshToFile(mf, "D:\\face_!.obj");
        ...
        Destroy(g);
           ...
           if (Event.current.keyCode == KeyCode.V)
           {
              ...
              for (int i = 0; i < lndmrks.Count; i++)
              {
                _verts.Add(Vector3.Scale(lndmrks[i], new Vector3(1.68f, 1, 1 )));
              }
              HelperScriptsForDebugging.SaveVector3ToCSV(_verts.ToArray(),"D:\\DEV\\new_verts.csv");
    ...
    void Update()
  {
    if (_loadVertsCSV.Length > 0)
    {
      try
      {
        var v3 = HelperScriptsForDebugging.LoadVector3FromCSV(_loadVertsCSV);
        _meshFilter.mesh.SetVertices(v3);
  ...
  public static async void MeshToFile(MeshFilter mf, string filename)
  {


    string r = MeshToString(mf);



    using StreamWriter file = new StreamWriter(filename);
    await file.WriteLineAsync(r);

  }



 // public async void SaveMesh(List<Mediapipe.NormalizedLandmarkList> multiFaceLandmarks){
